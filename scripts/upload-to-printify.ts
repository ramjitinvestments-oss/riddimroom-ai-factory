 ...
 *
 * **Remote duplicate guard (added 2026-08-05, post-incident).** The
 * "already uploaded, skip" check above only looks at the local
 * `<stem>.printify.json` file. That file lives under `designs/processed/`,
 * which is gitignored and never committed — on a fresh checkout (e.g.
 * every GitHub Actions run) it doesn't exist even for a design that was
 * already successfully uploaded in a prior run. This is exactly how the
 * 2026-08-05 incident produced 27 duplicate Printify products across 3
 * retries of the same batch: each retry's fresh runner had no local record
 * of the previous attempts' successes, so it re-created every one of them.
 * Before creating a new product, this script now calls
 * `PrintifyProvider.findProductIdByTitle()` against the configured shop. If
 * a product with this exact title already exists remotely, the batch stops
 * for that item with a clear error instead of silently creating a second
 * one — "never recreate a product that already exists" from a lookup
 * failure is treated the same as any other production-safe stop: it needs
 * a human to reconcile (was this expected? does the local job need its
 * `<stem>.printify.json` regenerated by hand to point at the existing
 * remote id?), not an automatic guess in either direction.
 *
...
export async function uploadApprovedArtworkToPrintify(
  ...
  for (const pngPath of pngPaths) {
    ...
    const duplicateCheck = await printify.findProductIdByTitle(product.title);
    if (!duplicateCheck.ok) {
      stoppedDueTo = {
        sourcePath: pngPath,
        jobId: product.jobId,
        reason: "Printify duplicate-check (findProductIdByTitle) failed",
        details: [duplicateCheck.error.message],
      };
      break;
    }
    if (duplicateCheck.value !== null) {
      stoppedDueTo = {
        sourcePath: pngPath,
        jobId: product.jobId,
        reason:
          `A Printify product titled "${product.title}" already exists in the configured shop ` +
          `(id ${duplicateCheck.value}), but there is no local ${outputs.printify} record of it. Refusing to ` +
          `create a duplicate. Reconcile by hand: either this design was already uploaded in a prior run and ` +
          `${outputs.printify} needs to be regenerated to point at ${duplicateCheck.value}, or this is a genuine ` +
          `title collision that needs a human decision.`,
        details: [],
      };
      break;
    }
