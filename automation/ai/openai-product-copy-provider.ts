...
export interface OpenAiProductCopyProviderOptions {
...
export class OpenAiProductCopyProvider implements ProductCopyProvider {
  ...
  constructor(options: OpenAiProductCopyProviderOptions) {
  ...
  async generate(
    ...
    const jobLogger = this.logger.withJob(request.jobId, "product-copy");

    try {
      const copy = await jobLogger.time(
        "Generate Product Copy",
        () =>
          withRetry(() => this.callOpenAi(request), {
      ...
      return err(new ExternalServiceError("openai", "product copy generation failed", { cause: error }));
    }
  }

  private async callOpenAi(request: ProductCopyRequest): Promise<ProductCopy> {
    const imageDataUrl = `data:image/png;base64,${request.artworkPng.toString("base64")}`;
    ...
    try {
      response = await this.fetchImpl(OPENAI_CHAT_ENDPOINT, {
        ...
        body: JSON.stringify({
          model: this.model,
          messages: [
            { role: "system", content: buildProductCopySystemPrompt() },
            {
              role: "user",
              content: [
                { type: "text", text: buildProductCopyUserPrompt(request.brief) },
          ...
          response_format: {
            ...
            json_schema: {
              name: "product_copy",
              schema: PRODUCT_COPY_JSON_SCHEMA,
              strict: true,
            },
          },
        }),
    ...
    const validated = validateProductCopy(parsed);
    if (!validated.ok) {
